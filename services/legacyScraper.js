...
async function executeOverpassQuery(query, serverIndex = 0) {
  ...
  try {
    ...
    clearTimeout(timeoutId);
    if (
      (error.name === 'AbortError' || error.message.includes('504') || error.message.includes('timeout')) &&
      serverIndex < OVERPASS_SERVERS.length - 1
    ) {
      logWarning(`Changement serveur Overpass: ${server} -> ${OVERPASS_SERVERS[serverIndex + 1]}`);
